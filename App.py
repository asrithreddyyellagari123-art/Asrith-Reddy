...
st.set_page_config(
    page_title="Smart Resume Analyzer using Artificial Intelligence",
    page_icon='./Logo/SRA_Logo.png',
)


def run():
    st.title("Smart Resume Analyser using Artificial Intelligence")
        ...
        if not st.session_state['logged_in']:
            ...
            if st.button('Login'):
                if ad_user == 'machine_learning_hub' and ad_password == 'mlhub123':
                    ...
                    st.success("Welcome Asrith Reddy")
                    st.rerun()
                else:
            ...
            # Section to clear / refresh database
            st.subheader("🧹 **Clear / Reset User Data**")
            st.warning("Resetting the database will permanently delete all user records from the system.")
            confirm_clear = st.checkbox("I understand that this action is irreversible and I want to delete all stored user data.")
            ...
            if confirm_clear:
                if st.button("Clear All Data"):
                    try:
                        ...
                        connection.commit()
                        st.success("All user data has been successfully cleared!")
                        time.sleep(1)
                        st.rerun()
            ...
            try:
                ...
                plot_data = pd.DataFrame(columns=['Predicted_Field', 'User_level'])
